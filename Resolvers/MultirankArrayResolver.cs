...
namespace SerializeMachine.Resolvers
{
    public sealed class MultirankArrayResolver : IResolver
    {
        ...
        private const string XML_ATTRIBUTE_RANK = "RANK";
        ...
        public override void Deserialzie(XElement serialized, ref object instance)
        {
            ...
            var indexator = new int[resolveArray.Rank];
        ...
        public override void Serialize(XElement serialized, object instance)
        {
            ...
            SetRank(serialized, resolveArray.Rank);
            SetDimensions(serialized, resolveArray);
        }

        protected internal override object GetTemplateInstance(XElement serializedObject)
        {
            var rank = GetRank(serializedObject);
            var dimensions = GetDimensions(serializedObject, rank);

            return Array.CreateInstance(ElementType, dimensions);
        }

        int GetRank(XElement serializedObject)
        {
            var rankAttribute = serializedObject.Attribute(XML_ATTRIBUTE_RANK);
            if (rankAttribute == null)
                throw new SerializationException($"MultirankArrayResolver. Rank the array read was failed: attribute <{XML_ATTRIBUTE_RANK}> not contains");

            int rank;
            if (!int.TryParse(rankAttribute.Value, out rank))
                throw new SerializationException($"MultirankArrayResolver. Rank the array read was failed: value <{XML_ATTRIBUTE_RANK}> is not 32-bit integer number");
        ...
        void SetRank(XElement serializedObject, int rank)
        {
            serializedObject.Add(new XAttribute(XML_ATTRIBUTE_RANK, rank.ToString()));
        }
